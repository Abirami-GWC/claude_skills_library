...
Each skill folder can be zipped individually into a `.skill` file and installed independently via Claude's **Save Skill** flow, or the whole `Content-Copywriting-Skill-Library/` folder can be kept together for editing and version control, packaging skills out of it as needed.

## Status

| Skill | Status |
|---|---|
| Brand Voice Writer | SKILL.md in place — supporting folders (`references/`, `templates/`, etc.) to be merged in from existing build |
| Ad Copy Generator | SKILL.md in place — supporting folders to be merged in from existing build |
